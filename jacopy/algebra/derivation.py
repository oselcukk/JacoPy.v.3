...
def degree_of(
    ...
    * Otherwise, :class:`ValueError`.
    ...
    # (core.expr.Atom subclasses may carry a Degree ``degree``
    # attribute). Those that do not carry a degree on the instance,
    # like Symbol, fall through to the registry path below.
    if isinstance(expr, Atom):
        _deg = getattr(expr, "degree", None)
        if isinstance(_deg, Degree):
            return _deg
    ...
    if isinstance(expr, Wedge):
        total = Degree.const(0)
        for c in expr.children:
            # The Wedge lift: children exposing a ``wedge_degree``
            # (vector fields, their brackets) contribute their
            # multivector degree instead of their operator degree, so
            # X ∧ Y is the bivector it is (degree 2, not 0).
            lift = getattr(c, "wedge_degree", None)
            total = total + (lift if isinstance(lift, Degree) else degree_of(c, registry))
        return total
    # Indexed sum: a finite sum of same-degree terms — the degree is
    # the body's (Phase 4.D.2; the bound index does not grade).
    from jacopy.core.indexed_sum import IndexedSum  # noqa: WPS433
    if isinstance(expr, IndexedSum):
        return degree_of(expr.body, registry)
    # Tensor product: same degree law ``|a ⊗ b| = |a| + |b|``.
    from jacopy.core.tensor_product import TensorProduct  # noqa: WPS433
    if isinstance(expr, TensorProduct):
        total = Degree.const(0)
        for c in expr.children:
            total = total + degree_of(c, registry)
        return total
    # (Anti)symmetrization preserves degree: ``|Sym(T)| = |Alt(T)| = |T|``.
    from jacopy.core.symmetrize import (  # noqa: WPS433
        Antisymmetrization,
        Symmetrization,
    )
    if isinstance(expr, (Symmetrization, Antisymmetrization)):
        return degree_of(expr.arg, registry)
    # Hodge star: ``|⋆ω| = n − |ω|`` (n = bundle dimension).
    from jacopy.core.hodge import HodgeStar  # noqa: WPS433
    if isinstance(expr, HodgeStar):
        return expr.dim - degree_of(expr.arg, registry)
    ...
    if isinstance(expr, BracketApply):
        # A bracket living in a different grading (e.g. the
        # Schouten-Nijenhuis multivector grading, where a vector field
        # counts 1, not its operator degree 0) supplies its own
        # apply_degree hook; the |a| + |b| + bracket.degree default is
        # only correct for brackets graded in the operator grading.
        hook = getattr(expr.bracket, "apply_degree", None)
        if hook is not None:
            return hook(expr.a, expr.b, registry)
        return (
    ...
    # inputs. ``Pairing`` is a core evaluation primitive
    # (``core.pairing``); the late import keeps the symmetry with
    # ``MultiEval`` and keeps the algebra→core direction clean.
    from jacopy.core.pairing import Pairing  # noqa: WPS433
