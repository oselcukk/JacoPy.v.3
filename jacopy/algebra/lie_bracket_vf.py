...
class LieBracketVF(Derivation):
    ...
    def wedge_degree(self):
        """Multivector-grading contribution inside a Wedge: the Lie
        bracket of vector fields is again a 1-vector."""
    ...
    def rewritable_slots(self):
        """Engine slot protocol: rules may fire inside the bracket's
        operand slots (see ExpansionEngine.expand_once)."""
        return (self._X, self._Y)

    def with_slots(self, X: Expr, Y: Expr) -> "LieBracketVF":
