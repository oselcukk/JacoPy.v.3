...
* ``slot_kind="mixed"``, args are ``q`` covectors followed by ``r``
  vector fields; ``head`` is a (q, r)-type tensor. Used with
  ``alternating=False`` for plain multilinear tensor evaluation.
...
_ALLOWED_SLOT_KINDS = ("vector", "covector", "mixed")
