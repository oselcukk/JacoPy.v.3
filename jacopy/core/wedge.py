...
Earlier stages (Phase 17.E.4) deferred the dedicated Wedge node on the
...
input. So Phase 17.F.1.5 introduces this node.
...
def wedge(*args: Expr) -> Expr:
    """Build the wedge product ``α ∧ β ∧ …`` (smart constructor :meth:`Wedge.make`).

    Function-style helper; positioned symmetrically to
    ``tensor_product`` / ``symmetrize``. Flattens nested wedges, handles
    the ``0``/``1`` units, and collapses to a single factor.
    """
    return Wedge.make(*args)
